import { createContext, Fragment, useState, useRef, useEffect, } from "react";
import {
  Row,
  Col,
  Card,
...
import {
  ...
  getVillages,
  getPincodes,
} from "../../../../services/LGD/index";
import { Navigations } from "../../../Assessment/components";
import { RadioField, SelectField, TextField } from "../../../formik/Inputs";
...
const DetailsOfTheProposedInstitute = ({ step, setActive, refreshSteps, nav }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const { district_list, sub_district_list, village_list } = result.data;
      if (district_list) {
        setDistrictsList(prev => ({ ...prev, ['cmp_post_district']: district_list }));
      }
      if (sub_district_list) {
        setSubDistrictsList(prev => ({ ...prev, ['cmp_post_sub_district']: sub_district_list }));
      }
      if (village_list) {
        setVillageList(prev => ({ ...prev, ['cmp_post_village']: village_list }));
      }
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      if (confirmResult.isConfirmed) {
        try {
          ...
          await ap.setProposedInstDetails(step, formikRef.current.values, appId);
          console.log(resp);
  ...
  const OnApplicantEntityStateChange = async (val, index) => {
    // Clear Before Init 
    formikRef.current.setFieldValue('cmp_post_district', '');
    formikRef.current.setFieldValue('cmp_post_sub_district', '');
    formikRef.current.setFieldValue('cmp_post_village', '');
    formikRef.current.setFieldValue('cmp_post_pincode', '');
    try {
  ...
  const OnApplicantEntityDistrictChange = async (val, index) => {
    ...
    formikRef.current.setFieldValue('cmp_post_pincode', '');
    try {
      let selectedSate = formikRef.current.values.cmp_post_state;
      const subdistrictsData = await getSubdistricts(selectedSate, val);
      setSubDistrictsList(prev => ({ ...prev, [index]: subdistrictsData }));
    } catch (error) {
      console.error("Error fetching districts:", error);
    }
  }
  const ApplicantEntitySubDistrict = async (val, index) => {
    ...
    formikRef.current.setFieldValue('cmp_post_pincode', '');
    try {
      let stateCode = formikRef.current.values.cmp_post_state;
      let districtCode = formikRef.current.values.cmp_post_district;
      let subDistrictCode = formikRef.current.values.cmp_post_sub_district;
  ...
  const onApplicantEntityVillageChange = async (val, index) => {
    ...
    try {
      ...
      formikRef.current.setFieldValue("cmp_post_pincode", pincodesData?.pincode || null);
    } catch (error) {
      console.error("Error fetching districts:", error);
                      ...
                      <Form ref={formRef2} onSubmit={handleSubmit} validated>
                        <Row className="mb-3">
                          ...
                          <Col md={6}>
                            <TextField
                              mandatory
                              label="Name of the Applicant Institute"
                              name="name_of_applicant_institute"
                              type="text"
                              contextName="Stage1Form" size="lg"
                              onValueChange={(value, event) => {
                                const sanitized = value.replace(/[^a-zA-Z\s]/g, "");
                                event.target.value = sanitized;
                                setFieldValue("name_of_applicant_institute", sanitized);
                              }}
                            />
                          </Col>
                              ...
                              <div className="p-2">
                                <Form.Group >
                                  ...
                                  <div style={{ display: "flex" }}>
                                    <RadioField inline label={`Government`} name="type_of_institute" value={`Government`} contextName="Stage1Form" mandatory size="lg" />
                                    <RadioField inline label={`Private`} name="type_of_institute" value={`Private`} contextName="Stage1Form" mandatory size="lg" />
                                  </div>
                                  {touched.type_of_institute &&
                                ...
                                <Row className="mb-3">
                                  ...
                                  <Col md="4">
                                    <SelectField
                                      ...
                                      mandatory
                                      options={districtsList['cmp_post_district'] || []} // row-specific districts
                                      contextName="Stage1Form"
                                      onValueChange={(val) => OnApplicantEntityDistrictChange(val, 'cmp_post_sub_district')}
                                  ...
                                  <Col md={4}>
                                    <SelectField
                                      label="Sub-District"
                                      name="cmp_post_sub_district"
                                      mandatory
                                      options={subDistrictsList['cmp_post_sub_district'] || []} // row-specific districts
                                      contextName="Stage1Form"
                                      onValueChange={(val) => ApplicantEntitySubDistrict(val, 'cmp_post_village')}
                                  ...
                                  <Col md={4}>
                                    <TextField label="Town/City" name="comp_town_city" type="text" mandatory contextName="Stage1Form" size="lg" />
                                  </Col>
                                  <Col md={4}>
                                  ...
                                  <Col md={4}>
                                    <SelectField
                                      label="Village"
                                      name="cmp_post_village"
                                      mandatory
                                      options={villageList['cmp_post_village'] || []} // row-specific districts
                                      contextName="Stage1Form"
                                      onValueChange={(val) => {
                                        console.log("Selected State:", val);
                                        onApplicantEntityVillageChange(val, 'cmp_post_pincode');
                                      }}
                                  ...
                                  <Col md={4}>
                                    <TextField disabled label="Pincode Number" name="cmp_post_pincode" type="text" mandatory contextName="Stage1Form" size="lg" />
                                  </Col>
                                  <Col md={4}>
                                    <TextField label="Plot Number/Khasara Number/Gata Number" name="cmp_post_plot_number_khasara_number" type="text" mandatory contextName="Stage1Form" size="lg" />
                                  </Col>
                                  <Col md={4}>
                                    <TextField label="Landmark" name="cmp_post_landmark" type="text" mandatory contextName="Stage1Form" size="lg" />
                                  </Col>
                                </Row>
                              </Card.Body>
                            </Card>
                              ...
                              <Card.Body>
                                <Row className="mb-3">
                                  <Form.Group as={Col} md={4}>
                                    ...
                                    <div>
                                      <RadioField inline label={`Urban`} name="institute_location" value={`Urban`} contextName="Stage1Form" size="lg" />
                                      ...
                                      checked={
                                        values.institute_location === "Urban"
                                      }
                                      isInvalid={
                                            ...
                                            <>
                                              <div className="p-2 flex-fill">
                                                ...
                                                <FileField2
                                                  ...
                                                  accept=".pdf,.jpg,.png"
                                                  context={FormContext}
                                                  onClickViewFileButton={() => viewFile(values.Falls_Under_Hill_Area_Hill__Supporting_Doc)}
                                                />
                                            ...
                                            /> */}
                                             ...
                                             <FileField2
                                              label="If Yes, Upload Supporting Government Notification/Order/Circular"
                                                  ...
                                                  accept=".pdf,.jpg,.png"
                                                  context={FormContext}
                                                  onClickViewFileButton={() => viewFile(values.Falls_Under_Border_District__Supporting_Doc)}
                                                />
                            ...
                            <Card className="border border-info custom-card">
                              <Card.Header>
                                <div
                                  ...
                                  style={{ textTransform: "none" }}
                                >
                                  Coordinates of institute{" "}
                                  <span style={{ "font-weight": "normal" }}>
                                    (For example Delhi - 28.7041° N, 77.1025° E)
                                  </span>
                                </div>
                              </Card.Header>
                              <Card.Body>
                                <Row className="mb-3">
                                  ...
                                  <Col md={3}>
                                    <TextField
                                      ...
                                      onValueChange={(value, event) => {
                                        ...
                                        event.target.value = sanitized;
                                        setFieldValue("latitude", sanitized);
                                      }}
                                    />
                                  </Col>
                                  ...
                                  >
                                    <Form.Label>
                                      Latitude<span style={{ color: "red" }}> (in degree) *</span>
                                    </Form.Label>
                                  ...
                                  <Col md={3}>
                                    <TextField label={`Longitude (in degree)`}
                                      name="Longitude"
                                      type="text"
                                      mandatory
                                      contextName="Stage1Form"
                                      ...
                                      onValueChange={(value, event) => {
                                        const sanitized = value
                                          ...
                                          .replace(/(\..*)\./g, "$1"); // only one '.'
                                        event.target.value = sanitized;
                                        setFieldValue("Longitude", sanitized);
                                      }}
                                    />
                                  </Col>
                                  {false && (<Form.Group
          ...
          <Card className="custom-card shadow">
            <Card.Body>
              ...
              <Row className="mb-3">
                <Form.Group as={Col} md="6" >
                  <Form.Label>
                    Whether Applying Under Mini Skill Training Institute (MSTI)
                    Category?
                    <span style={{ color: "red" }}>
                      *{" "}
                      <i
