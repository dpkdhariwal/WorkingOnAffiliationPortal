...
export const MachineryToolEquipment = ({ isView = false, nav }) => {
  ...
  const loadData = async () => {
    try {
      // const resp = await ap.getMachineryToolEquipments(appId);
      // const data = resp.data;
      // setInitialValues(data);
      const resp2 = await ap.getStatusFillingMte(appId);
      const data2 = resp2.data;

      const data3 = data2.map((step) => ({ ...step, completed: step.status === C.SL.FILLED }));
      console.log(data3);
      setSteps(data3);


      const firstIndex = 0;                 // Always 0 for the first element
      const lastIndex = data2.length - 1;   // Length minus 1 for the last element
  ...
  const setStepContent = (step, stepIndex) => {
    console.log(step);
    return <ToolList key={stepIndex} nav={object(step, stepIndex)} step={step} setActive={goToSection} refreshSteps={loadData} />
  }
  const [currentStep, setCurrentStep] = useState(0);
  const handleStepClick = (step, index) => {
    console.log(step);
    if (step.stepStatus === C.SL.ACTIVE) {
      setActiveStep(index)
    }
  };
  ...
  return (
    <Fragment>
      ...
      {/* <Formik
        ...
        enableReinitialize
        validationSchema={C.st2.MachineryToolEquipment.ValSchema}
            ...
            <Card className="custom-card border border-primary">
              <Card.Header>
                <div className="card-title" style={{ textTransform: "none" }}>
                  <h5>Fill Machinery/Tools/Equipment Details</h5>
                </div>
              </Card.Header>
              <Card.Body>
                <FieldArray name="mte">
                  {({ push, remove }) => (
                    <Table >
                      ...
                      <tbody>
                        {values.mte.map((obj, index) => {
                          return (
                            <tr key={index}>
                              <td>{index + 1}</td>
                              <td>{obj?.tradeInfo?.trade_name}</td>
                              <td>{obj?.toolInfo?.Item_name}</td>
                              <td>{obj?.toolInfo?.Specification}</td>
                              <td>{obj?.toolInfo?.Quantity}</td>
                              <td>
                                <Field
                                  type="number"
                                  name={`mte[${index}].available_quantity`}
                                  as={Form.Control}
                                />
                                <div className="text-danger">
                                  <ErrorMessage name={`mte[${index}].available_quantity`} />
                                </div>
                              </td>
                            </tr>
                          )
              ...
              <Card.Footer>
                <Navigations nav={nav} onLast={() => { onNext(); }} />
              </Card.Footer>
            </Card>
          </Form>
        )}
...
export const ToolList = ({ step, isView = false, nav }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const { tradeId } = step;
      console.log(tradeId);
      const resp = await ap.getMachineryToolEquipments(appId, tradeId, { page, limit: pageSize });
      const data = resp.data;
      console.log(data);
        ...
        {({ handleSubmit, handleChange, setFieldValue, values, errors, touched }) => (
          <Form onSubmit={handleSubmit}>
            <Card className="custom-card border border-primary">
                            ...
                            <tr key={index}>
                              ...
                              {false && (
                                <td>
                                  ...
                                  <div className="text-danger">
                                    <ErrorMessage name={`mte[${index}].availability`} />
                                  </div>
                                </td>
                              )}
                              <td>
                                <Field
                                  disabled={values.mte[index].availability === "no"}
                                  type="number"
                                  name={`mte[${index}].available_quantity`}
                                  as={Form.Control}
                                />
              ...
              <Card.Footer>
                <Navigations nav={nav} onNext={() => { onNext(); }} onLast={() => { onNext(); }} lastLabel="Save & Next" />
              </Card.Footer>
            </Card>
          </Form>
        )}
