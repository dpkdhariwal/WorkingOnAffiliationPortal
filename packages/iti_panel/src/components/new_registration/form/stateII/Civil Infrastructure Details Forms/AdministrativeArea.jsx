...
export const AdministrativeArea = ({ steps, goPrevious, goNext }) => {
    ...
    const [initialValues, setInitialValues] = useState(C.st2.CivilInfra.administrative_areas.initialValue);
    ...
    const prepareToSave = async (values) => {
        try {
            ...
            await ap.saveParticularDetails(values, appId, C.CIC.ADMINISTRATIVE_AREA);
            // Close loader
            Swal.close();
    ...
    useEffect(() => {
        const currentStep = steps.subSteps.find(step => step.step === CIC.ADMINISTRATIVE_AREA);
        setCurrentStep(currentStep)
    }, [])
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const resp = await ap.getParticulars(appId, C.CIC.ADMINISTRATIVE_AREA);
                const data = resp.data;
                const newValues = { administrative_areas: data };
    ...
    return (
        <Formik
            ...
            validationSchema={C.st2.CivilInfra.administrative_areas.ValSchema}
            onSubmit={(values) => {
                prepareToSave(values);
            }}
        >
            {({ handleSubmit, handleChange, values, errors, touched, setFieldValue }) => (
                <FormikForm onSubmit={handleSubmit}>
                    <FormContext.Provider value={{ handleSubmit, handleChange, values, errors, touched, setFieldValue }}>
                        ...
                        <FieldArray name="administrative_areas">
                            {({ push, remove }) => (
                                <Card>
                                    <Card.Body>
                                        <div className="table-responsive">
                                            <Table >
                                                ...
                                                <tbody>
                                                    {values.administrative_areas.map((doc, index) => {
                                                        { console.log(doc) }
                                                        return (
                                                            <tr key={index}>
                                                                ...
                                                                <td>
                                                                    <Field
                                                                        type="number"
                                                                        name={`administrative_areas[${index}].available_area`}
                                                                        as={Form.Control}

                                                                    />
                                                                    <div className="text-danger">
                                                                        <ErrorMessage name={`administrative_areas[${index}].available_area`} />
                                                                    </div>
                                                                </td>
                                                                <td>
                                                                    <FileField2
                                                                        //   label="If Yes, Upload Supporting Government Notification/Order/Circular"
                                                                        name={`administrative_areas[${index}].document`}
                                                                        mandatory
                                                                        accept=".pdf,.jpg,.png"
                                                                        context={FormContext}
                                                                        onClickViewFileButton={() => viewFile(values.administrative_areas[index].document)}
                                                                    />
                                                                </td>
                                                            </tr>
                                                        )
