...
import { GoToStageIAssessmentToUploadDocs, GoToStageIIAssessmentToUploadDocs,  GoToStageIForm, GoToStageIAssessment, GoToNOCGenerateForm, GoToStageIIForm, GoToStageIIAssessment, GoToStageIIStaffDetailForm, GoToInspectionSlotSelection, GoToBatchCreattion } from "../action/allActions";
...
export const PendingStep = ({ info, variant }) => {
          ...
          case STAGE_II__ASSESSMENT_ON_PROGRESS:
            switch (authUser.userType) {
              case 'state_assessor':
                switch (info?.aStatus?.pendingAt) {
                  case C.SL.PENDING_AT_ASSESSOR:
                    return <GoToStageIIAssessment info={info} />;
                  default:
                    break;
                }
                break
              case 'applicant':
                switch (info?.aStatus?.pendingAt) {
                  case C.SL.PENDING_AT_APPLICANT:
                  case C.SL.APPLICANT:
                    return <GoToStageIIAssessmentToUploadDocs info={info} />;
                  default:
                    return 'NA';
                }
              default:
  ...
  const getSetstatus = (info) => {
    switch (info.step) {
      ...
      //   }
      case STAGE_I__ASSESSMENT:
        if (info?.aStatus?.pendingAt) {
          return <Badge bg="warning" >{pendingAtLabel[info?.aStatus?.pendingAt]}</Badge>
        }
        break;
      // case NOC_ISSUANCE:
      //   switch (info.status) {
  ...
  const pendingAtLabel = {
    [C.SL.PENDING_AT_ASSESSOR]: "Assessor",
    [C.SL.PENDING_AT_APPLICANT]: "@Applicant",
    [C.SL.APPLICANT]: "Applicant",
    [C.SL.STATE]: "State Authority",
    // add more as needed
  };
  return (
    <Card
    ...
    >
      <Card.Body style={{ padding: "5px" }}>
        <div className="d-flex justify-content-between mb-3">
          ...
          <div>{getSetstatus(info)}</div>
        </div>
        <h6 className="card-title fw-semibold mb-2">{t(`AppFlow.${info.step}.status.${variant == "inProgress" ? "on-progress" : 'pending'}.${'COMMON'}`)}</h6>
      </Card.Body>
