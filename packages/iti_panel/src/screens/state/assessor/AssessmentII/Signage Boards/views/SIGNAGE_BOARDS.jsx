                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                    {values.SIGNAGE_BOARDS.replied === false && (
                                        
                                        <DocumentPhoto FormContext={FormContext} particular={formatLabel('SIGNAGE_BOARDS')} documentUrl={values?.SIGNAGE_BOARDS?.initial_detail?.document} />   
                                )}

                                    {values.SIGNAGE_BOARDS.replied === true && (<DocumentPhoto FormContext={FormContext} particular={formatLabel('SIGNAGE_BOARDS')} documentUrl={values?.SIGNAGE_BOARDS?.initial_detail?.document} />)}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}
                            {values.SIGNAGE_BOARDS.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputPhoto
                                        particular="SIGNAGE_BOARDS"
                                        document_field={`REVISED_SIGNAGE_BOARDS.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            ...
                            {values.usertype === C.SL.APPLICANT
                                && values.SIGNAGE_BOARDS?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`SIGNAGE_BOARDS`}
                                    as_per_norms={values.SIGNAGE_BOARDS?.as_per_norms}
                                    reason={values.SIGNAGE_BOARDS?.reason}
                                    assessor_remark={values.SIGNAGE_BOARDS?.assessor_remark} />)}
                                ...
                                <Card.Body>
                                        ...
                                        <Form.Group>
                                            ...
                                            <div role="group" aria-labelledby="as_per_norms-group">
                                                <label className="me-3">
                                                    <Field
                                                        type="radio"
                                                        name="SIGNAGE_BOARDS.as_per_norms"
                                                        value="yes"
                                    ...
                                    {values.SIGNAGE_BOARDS.as_per_norms === "no" && (
                                        <Row className="mb-3">
                                            ...
                                            {values.SIGNAGE_BOARDS.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="SIGNAGE_BOARDS.assessor_comments"
                                                        value={values.SIGNAGE_BOARDS.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.SIGNAGE_BOARDS?.assessor_comments &&
                                                            errors.SIGNAGE_BOARDS?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.SIGNAGE_BOARDS?.assessor_comments &&
                                                        errors.SIGNAGE_BOARDS?.assessor_comments && (
