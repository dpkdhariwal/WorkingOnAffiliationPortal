                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                    {values.STAFF_ROOM.replied === false && (<ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} document={info?.initial_detail?.document} />)}

                                    {values.STAFF_ROOM.replied === true && ('Show Latest')}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}

                            {values.STAFF_ROOM.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputArea
                                        particular="STAFF_ROOM"
                                        required_area={values?.STAFF_ROOM?.initial_detail?.required_area}
                                        required_area_fieldname={`REVISED_STAFF_ROOM.avaialble_area`}
                                        document_field={`REVISED_STAFF_ROOM.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            ...
                            {values.usertype === C.SL.APPLICANT
                                && values.STAFF_ROOM?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`STAFF_ROOM`}
                                    as_per_norms={values.STAFF_ROOM?.as_per_norms}
                                    reason={values.STAFF_ROOM?.reason}
                                    assessor_remark={values.STAFF_ROOM?.assessor_remark} />)}
                                ...
                                <Card.Body>
                                    ...
                                    {values.STAFF_ROOM.as_per_norms === "no" && (
                                        <Row className="mb-3">
                                            ...
                                            {values.STAFF_ROOM.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="STAFF_ROOM.assessor_comments"
                                                        value={values.STAFF_ROOM.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.STAFF_ROOM?.assessor_comments &&
                                                            errors.STAFF_ROOM?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.STAFF_ROOM?.assessor_comments &&
                                                        errors.STAFF_ROOM?.assessor_comments && (
