                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                    {values.LIBRARY.replied === false && (<ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} document={info?.initial_detail?.document} />)}

                                    {values.LIBRARY.replied === true && ('Show Latest')}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}

                            {values.LIBRARY.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputArea
                                        particular="LIBRARY"
                                        required_area={values?.LIBRARY?.initial_detail?.required_area}
                                        required_area_fieldname={`REVISED_LIBRARY.avaialble_area`}
                                        document_field={`REVISED_LIBRARY.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.APPLICANT
                                && values.LIBRARY?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`LIBRARY`}
                                    as_per_norms={values.LIBRARY?.as_per_norms}
                                    reason={values.LIBRARY?.reason}
                                    assessor_remark={values.LIBRARY?.assessor_remark} />)}
                                            ...
                                            {values.LIBRARY.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="LIBRARY.assessor_comments"
                                                        value={values.LIBRARY.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.LIBRARY?.assessor_comments &&
                                                            errors.LIBRARY?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.LIBRARY?.assessor_comments &&
                                                        errors.LIBRARY?.assessor_comments && (
