                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (
                                <Card ref={divRef} className="border border-2  card custom-card  card">
                                    <Card.Body>
                                        {console.log(info)}
                                        {values.IT_LAB.replied === false && (<ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} document={info?.initial_detail?.document} />)}

                                        {values.IT_LAB.replied === true && ('Show Latest')}
                            ...
                            {values.IT_LAB.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputArea
                                        particular="IT_LAB"
                                        required_area={values?.IT_LAB?.initial_detail?.required_area}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.APPLICANT
                                && values.IT_LAB?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`IT_LAB`}
                                    as_per_norms={values.IT_LAB?.as_per_norms}
                                    reason={values.IT_LAB?.reason}
                                    assessor_remark={values.IT_LAB?.assessor_remark} />)}
                                            ...
                                            {values.IT_LAB.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="IT_LAB.assessor_comments"
                                                        value={values.IT_LAB.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.IT_LAB?.assessor_comments &&
                                                            errors.IT_LAB?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.IT_LAB?.assessor_comments &&
                                                        errors.IT_LAB?.assessor_comments && (
