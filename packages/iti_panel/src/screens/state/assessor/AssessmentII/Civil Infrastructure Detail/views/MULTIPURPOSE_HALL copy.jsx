...
export const Classrooms_asmt = ({ title, info, FormContext }) => {
    ...
    return (
        <>
                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                               {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                     {values.CLASSROOMS.replied === false && (<ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} document={info?.initial_detail?.document}/>)}

                                     {values.CLASSROOMS.replied === true && ('Show Latest')}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}

                              {values.CLASSROOMS.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                                            <Card.Body>
                                                                 <RevisedInputArea
                                                                                                        required_area_fieldname={`REVISED_IT_LAB.avaialble_area`}
                                                                                                        document_field={`REVISED_IT_LAB.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {/* style={{ height: `${height}px` }} */}
                               {values.usertype === C.SL.APPLICANT
                                                            && values.CLASSROOMS?.as_per_norms === "no" &&
                                                            (<ViewStateRemark title={`CLASSROOMS`}
                                                                as_per_norms={values.CLASSROOMS?.as_per_norms}
                                                                reason={values.CLASSROOMS?.reason}
                                                                assessor_remark={values.CLASSROOMS?.assessor_remark} />)}
            ...
            <option key={i} value={lang.value}>{lang.label}</option>
                                            ...
                                            {values.CLASSROOMS.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="CLASSROOMS.assessor_comments"
                                                        value={values.CLASSROOMS.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.CLASSROOMS?.assessor_comments &&
                                                            errors.CLASSROOMS?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.CLASSROOMS?.assessor_comments &&
                                                        errors.CLASSROOMS?.assessor_comments && (
