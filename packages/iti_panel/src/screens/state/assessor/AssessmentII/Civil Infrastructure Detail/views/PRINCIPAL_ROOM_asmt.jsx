                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                    {values.PRINCIPAL_ROOM.replied === false && (
                                        <ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} document={info?.initial_detail?.document} />)}

                                    {values.PRINCIPAL_ROOM.replied === true && ('Show Latest')}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}

                            {values.PRINCIPAL_ROOM.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputArea
                                        particular="PRINCIPAL_ROOM"
                                        required_area={values?.PRINCIPAL_ROOM?.initial_detail?.required_area}
                                        required_area_fieldname={`REVISED_PRINCIPAL_ROOM.avaialble_area`}
                                        document_field={`REVISED_PRINCIPAL_ROOM.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            ...
                            {values.usertype === C.SL.APPLICANT
                                && values.PRINCIPAL_ROOM?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`PRINCIPAL_ROOM`}
                                    as_per_norms={values.PRINCIPAL_ROOM?.as_per_norms}
                                    reason={values.PRINCIPAL_ROOM?.reason}
                                    assessor_remark={values.PRINCIPAL_ROOM?.assessor_remark} />)}
                                ...
                                <Card.Body>
                                        ...
                                        <Form.Group>
                                            ...
                                            <div role="group" aria-labelledby="as_per_norms-group">
                                                <label className="me-3">
                                                    <Field
                                                        type="radio"
                                                        name="PRINCIPAL_ROOM.as_per_norms"
                                                        value="yes"
                                    ...
                                    {values.PRINCIPAL_ROOM.as_per_norms === "no" && (
                                        <Row className="mb-3">
                                            ...
                                            {values.PRINCIPAL_ROOM.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="PRINCIPAL_ROOM.assessor_comments"
                                                        value={values.PRINCIPAL_ROOM.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.PRINCIPAL_ROOM?.assessor_comments &&
                                                            errors.PRINCIPAL_ROOM?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.PRINCIPAL_ROOM?.assessor_comments &&
                                                        errors.PRINCIPAL_ROOM?.assessor_comments && (
