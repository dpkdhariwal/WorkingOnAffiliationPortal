...
import { ViewAreaWorkshop } from "@/components/Assessment/ViewAreaWorkshop";
import { RevisedInputArea } from "@/components/Assessment/RevisedInputArea";

export const Workshops_asmt = ({ title, info, FormContext }) => {
                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                               {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                     {values.WORKSHOPS.replied === false && (<ViewAreaWorkshop info={values.WORKSHOPS} />)}

                                     {values.WORKSHOPS.replied === true && ('Show Latest')}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}

                              {values.WORKSHOPS.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                                            <Card.Body>
                                                                <RevisedInputArea
                                                                                                            required_area_fieldname={`REVISED_WORKSHOPS.avaialble_area`}
                                                                                                            document_field={`REVISED_WORKSHOPS.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {/* style={{ height: `${height}px` }} */}
                               ...
                               {values.usertype === C.SL.APPLICANT
                                                            && values.WORKSHOPS?.as_per_norms === "no" &&
                                                            (<ViewStateRemark title={`WORKSHOPS`}
                                                                as_per_norms={values.WORKSHOPS?.as_per_norms}
                                                                reason={values.WORKSHOPS?.reason}
                                                                assessor_remark={values.WORKSHOPS?.assessor_remark} />)}
                                            ...
                                            {values.WORKSHOPS.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="WORKSHOPS.assessor_comments"
                                                        value={values.WORKSHOPS.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.WORKSHOPS?.assessor_comments &&
                                                            errors.WORKSHOPS?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.WORKSHOPS?.assessor_comments &&
                                                        errors.WORKSHOPS?.assessor_comments && (
