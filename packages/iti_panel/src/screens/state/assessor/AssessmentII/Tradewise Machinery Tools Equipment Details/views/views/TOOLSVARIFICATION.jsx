...
export const Comp = ({ step, title, info, index, FormContext }) => {
    { console.log(info) }
    ...
    const [pageSize] = useState(25); // you can make this dynamic if needed
    ...
    return (
        <>
            {console.log(info?.review_counts)}
            <div className={`border ${info?.review_counts?.total == info?.review_counts?.as_per_norms ? 'border-success border-2' : 'border-primary'} d-flex justify-content-between`} style={{ padding: 12, backgroundColor: "antiquewhite", borderRadius: 10, marginTop: "10px" }}>
                <h5>{info.stepTitle}</h5>
                <div className="border border-primary d-flex justify-content-between gap-3">
                    {console.log(values?.usertype)}
                    {C.AdminList.includes(values?.usertype) === true && (
                    ...
                    {values?.usertype === C.UserType.APPLICANT && (
                        <div className="border-primary d-flex gap-3 justify-content-between"
                            style={{ alignContent: "center", alignItems: "center", padding: 7 }}>
                            Total: {info?.review_counts?.total} | Replied: {info?.review_counts?.applicant_total_replied} | Pending: {info?.review_counts?.total_pending_at_applicant}
                        </div>
                    )}

                    
                    {C.AdminList.includes(values?.usertype) && (
                        <Button onClick={() => openVerificationWindow()} className="btn btn-primary">
                            Verifications
                        </Button>
                    )}

                    {/* {info?.review_counts?.total == info?.review_counts?.as_per_norms && (
...
const SelectCategoryModal = (props) => {
    ...
    const loadData = async () => {
        try {
            ...
            const data = resp.data;
            const { mte, usertype } = data;

            const updated = { ...initialValues, toolList: mte, usertype: usertype };
            setInitialValues(updated);
            setTools(mte);
            setTotalRows(data.totalRecords);
        ...
        try {
            ...
            if (formsRef.current.isValid === true) {
                try {
                    ...
                    const resp = await ap.save_revised_tool_qty(appId, formsRef.current.values, props.step);
    ...
    return (
        <Modal {...props} size="xl" fullscreen centered>
                                        ...
                                        <Table className="card-table table-vcenter  mb-0 border dashboard-table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    {C.AdminList.includes(values?.usertype) && (<th style={{ width: '10%' }}>as per norms?</th>)}
                                                ...
                                                {values?.toolList?.map((obj, index) => {
                                                    ...
                                                    return (
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                {C.AdminList.includes(values?.usertype) && (<span>{obj?.available_quantity}</span>)}

                                                                {values?.usertype == C.UserType.APPLICANT && (
                                                                    <div>
                                                                        <Field
                                                                            type="number"
                                                                            name={`toolList.${index}.revised_qty`}
                                                                            as={Form.Control}
                                                                        />
                                                                        <div className="text-danger">
                                                                            <ErrorMessage name={`toolList.${index}.revised_qty`} />
                                                                        </div>
                                                                    </div>
                                                                )}
            ...
            <Modal.Footer className="d-flex justify-content-between mb-3">
                ...
                {console.log(initialValues.usertype)}

                {initialValues.usertype == C.UserType.APPLICANT ? (
                    <Button onClick={() => { onNext() }} disabled={processing} variant="primary">
                        {processing === true && (
                            <Spinner
                                as="span"
                                animation="grow"
                                ...
                                aria-hidden="true"
                            />)}
                        {processing === true ? (` Saving...`) : `Submit & Next`}
                    </Button>
                ) : (
