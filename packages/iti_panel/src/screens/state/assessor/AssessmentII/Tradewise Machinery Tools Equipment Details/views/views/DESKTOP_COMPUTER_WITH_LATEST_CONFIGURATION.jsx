...
import { RevisedInputQty } from "@/components/Assessment/RevisedInputQty";

export const Comp = ({ title, info, index, FormContext }) => {
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (
                                <Card ref={divRef} className="border border-2  card custom-card  card">
                                    <Card.Body>
                                        {console.log(info)}
                                        {values.itLab[index].replied === false && (
                                        ...
                                        {values.itLab[index].replied === true && ('Show Latest')}
                                    </Card.Body>
                                </Card>)}
                            {values?.itLab[index]?.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputQty
                                        fieldName={`itLab.${index}.revised_qty`}
                                        particular={title}
                                        required_qty={values?.itLab[index]?.required_qty} />
                                </Card.Body>
                            </Card>)}
                        </Col>
                        <Col xl={6} lg={6} md={6} sm={6}>
                            ...
                            {values.usertype === C.SL.APPLICANT
                                ...
                                (<ViewStateRemark title={title}
                                    as_per_norms={values.itLab[index].as_per_norms}
                                    reason={values.itLab[index].reason}
                                    assessor_remark={values.itLab[index].assessor_remark} />)}
