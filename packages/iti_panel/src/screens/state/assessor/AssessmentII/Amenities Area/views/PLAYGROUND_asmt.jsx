                    ...
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {values.usertype === C.SL.ASSESSOR && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    {console.log(info)}
                                    {values.PLAYGROUND.replied === false && (<ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} />)}

                                    {values.PLAYGROUND.replied === true && ('Show Latest')}

                                    {console.log(info)}
                                </Card.Body>
                            </Card>)}

                            {values.PLAYGROUND.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputPhoto
                                        particular="PLAYGROUND"
                                        document_field={`REVISED_PLAYGROUND.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            ...
                            {values.usertype === C.SL.APPLICANT
                                && values.PLAYGROUND?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`PLAYGROUND`}
                                    as_per_norms={values.PLAYGROUND?.as_per_norms}
                                    reason={values.PLAYGROUND?.reason}
                                    assessor_remark={values.PLAYGROUND?.assessor_remark} />)}
                                ...
                                <Card.Body>
                                    ...
                                    {values.PLAYGROUND.as_per_norms === "no" && (
                                        <Row className="mb-3">
                                            ...
                                            {values.PLAYGROUND.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="PLAYGROUND.assessor_comments"
                                                        value={values.PLAYGROUND.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.PLAYGROUND?.assessor_comments &&
                                                            errors.PLAYGROUND?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.PLAYGROUND?.assessor_comments &&
                                                        errors.PLAYGROUND?.assessor_comments && (
