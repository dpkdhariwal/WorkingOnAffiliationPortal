                ...
                <Card.Body style={{ padding: "5px" }} >
                    <Row style={{
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            {console.log(values.AVAILABILITY_OF_STAIRCASES)};
                            {values.usertype === C.SL.ASSESSOR && (
                                <Card ref={divRef} className="border border-2  card custom-card  card">
                                    <Card.Body>
                                        {console.log(info)}
                                        {values.AVAILABILITY_OF_STAIRCASES.replied === false && (<ViewArea required_area={info?.initial_detail?.required_area} totalAvailableArea={info?.initial_detail?.totalAvailableArea} FormContext={FormContext} />)}

                                        {values.AVAILABILITY_OF_STAIRCASES.replied === true && ('Show Latest')}

                                        {console.log(info)}
                                    </Card.Body>
                                </Card>)}

                            {values.AVAILABILITY_OF_STAIRCASES.as_per_norms == C.SL.NO && values.usertype === C.SL.APPLICANT && (<Card ref={divRef} className="border border-2  card custom-card  card">
                                <Card.Body>
                                    <RevisedInputPhoto
                                        particular="AVAILABILITY_OF_STAIRCASES"
                                        document_field={`REVISED_AVAILABILITY_OF_STAIRCASES.document`}
                        ...
                        <Col xl={6} lg={6} md={6} sm={6}>
                            ...
                            {values.usertype === C.SL.APPLICANT
                                && values.AVAILABILITY_OF_STAIRCASES?.as_per_norms === "no" &&
                                (<ViewStateRemark title={`AVAILABILITY_OF_STAIRCASES`}
                                    as_per_norms={values.AVAILABILITY_OF_STAIRCASES?.as_per_norms}
                                    reason={values.AVAILABILITY_OF_STAIRCASES?.reason}
                                    assessor_remark={values.AVAILABILITY_OF_STAIRCASES?.assessor_remark} />)}
                                        ...
                                        <Form.Group>
                                            ...
                                            <div role="group" aria-labelledby="as_per_norms-group">
                                                <label className="me-3">
                                                    <Field
                                                        type="radio"
                                                        name="AVAILABILITY_OF_STAIRCASES.as_per_norms"
                                                        value="yes"
                                    ...
                                    {values.AVAILABILITY_OF_STAIRCASES.as_per_norms === "no" && (
                                        <Row className="mb-3">
                                            <Form.Group as={Col} md="12">
                                                ...
                                                <Field
                                                    name="AVAILABILITY_OF_STAIRCASES.reason"
                                                    as="select"
                                            ...
                                            {values.AVAILABILITY_OF_STAIRCASES.reason === "other" && (
                                                <Form.Group as={Col} md="12" style={{ marginTop: "1rem" }}>
                                                    ...
                                                    <Form.Control
                                                        as="textarea"
                                                        rows={3}
                                                        name="AVAILABILITY_OF_STAIRCASES.assessor_comments"
                                                        value={values.AVAILABILITY_OF_STAIRCASES.assessor_comments}
                                                        onChange={handleChange}
                                                        className={`form-control ${touched.AVAILABILITY_OF_STAIRCASES?.assessor_comments &&
                                                            errors.AVAILABILITY_OF_STAIRCASES?.assessor_comments
                                                            ? "is-invalid"
                                                            : ""
                                                            }`}
                                                    />
                                                    {touched.AVAILABILITY_OF_STAIRCASES?.assessor_comments &&
                                                        errors.AVAILABILITY_OF_STAIRCASES?.assessor_comments && (
