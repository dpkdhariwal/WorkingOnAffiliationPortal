...
export const LandDocuments = ({ steps, step, view: viewProp = false, isView = false, nav }) => {
  ...
  const onNext = async () => {
    ...
    try {
      for (const id in formsRef.current) {
        ...
        console.log(changeInForm, getData, editMode, refNo);
        changeArray.push(changeInForm);
        if (changeInForm == true) {
          let formInstance = getData();
          if (typeof formInstance.current == 'object') {
            console.log(typeof formInstance.current);
    ...
    if (changeArray.every(v => !v)) {
      ...
      if (isFormValid.every(v => v)) {
        ...
        Swal.fire({
          icon: "warning",
          title: "Address Deficiencies Before Next",
  ...
  const loadData = async () => {
    try {
      ...
      addItem({ row: null, title: "Land Deatil", comp: <LandInfoView /> });
  ...
  return (
    <FormContext.Provider value={{ registerForm, unregisterForm }}>
      ...
      <div key={0} style={{ backgroundColor: "rgb(245, 245, 245)", margin: "10px 0px 0px", borderRadius: 6, borderStyle: "dashed", borderWidth: "thin", padding: "10px", }}>
        ...
        <div>
          <Accordion className="customized-accordion accordions-items-seperate" alwaysOpen
          >
            {vCompList.map((c, idx) => {
              return (
      ...
      {isView == false && <Navigations nav={nav} onNext={onNext} nextLabel="Save & Next" />}
...
export const PossessionOfLandForApplicant = ({ data, step, view: viewProp = false, isView = false }) => {
  ...
  const resubmitLandDocuments = async () => {
    console.log(data);
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Cleared",
        text: "Please resubmit land Documents",
        timer: 2000,
      ...
      {data?.da_status == C.SL.REPLIED ? (
        ...
        <Card className="bg-body-tertiary border border-2 border-danger card custom-card shadow-danger shadow-size-small">
          <Card.Header className="d-flex justify-content-between">
            <Modal.Title as="h6">New Land Documents</Modal.Title>
            <Button onClick={() => { resubmitLandDocuments() }} variant="outline-warning" size="sm">Re-Submit</Button>
          </Card.Header>
          <Card.Body>
            <Table
            ...
            <thead>
              <tr>
                <th style={{ border: "1px solid black", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <Button variant="warning" size="sm">View Trail</Button>
                </th>
              </tr>
            </thead>
            <tbody>
                      ...
                      {land_documents?.map((item, index) => {
                        ...
                        return (
                          <tr key={index}>
                            ...
                            <td style={{ border: "1px solid black" }}>
                              <Button className="btn-w-lg" variant="info" size="sm" onClick={() => viewFile(item.notarised_document)}>View File</Button>
                            </td>
            ...
            {reviewStatus == C.SL.PENDING ? (
              <Formik
                ...
                validationSchema={yup.object().shape({
                  ...
                  assessor_comments: yup.string().when("as_per_norms", { is: (as_per_norms, reason) => as_per_norms === "no" || reason === "other", then: () => yup.string().required("Please provide your comments"), otherwise: () => yup.string().notRequired(), }),
                })}
              ...
              <>
                ...
                <Card
                  className="border border-2 border-success shadow-success  card custom-card shadow-size-small  card"
                  style={
                    initValue?.as_per_norms == "yes" || formData?.as_per_norms == "yes"
                      ? { backgroundColor: "#d6f3e0" }
                      : { backgroundColor: "#f3d6d6" }
                  }
                >
                  <Card.Header>
                    ...
                    <div className="ms-auto  d-flex">
                      {formatedDate(remarkInfo?.updateDate)}
                    </div>
                  </Card.Header>
                  <Card.Body style={{ paddingBottom: '0px', paddingTop: '4px' }} >
                    <Row className="mb-3">
