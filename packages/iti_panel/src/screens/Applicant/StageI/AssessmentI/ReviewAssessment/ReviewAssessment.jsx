...
export const ReviewAssessment = ({ step, nav }) => {
  ...
  const onNext = async () => {
    ...
    try {
      await st.setSendStageIApplicationToState(appId, step.assessment_id);
  ...
  const loadData = async () => {
    try {
      ...
      const resp = await st.getDocumentUploadStateforStageI(appId, step.assessment_id);
                ...
                <Form onSubmit={handleSubmit} validated>
                  <Card className="custom-card border border-success" style={{ padding: "0px" }}>
                    {resp.action === C.SL.SEND_TO_ASSESSOR && (<Card.Body style={{ padding: "5px" }} >
                      <p>Dear Applicant,</p>
                      <p>{`You have successfully uploaded your documents to address the non-conformity. To proceed, please click the "Forward for Assessment" button to resubmit your application for further evaluation.`}</p>
                    </Card.Body>)}

                    {resp.action === C.SL.REMOVE_DEFICIENCIES_FIRST && (<Card.Body style={{ padding: "5px" }} >
                      Please Remove Deficincies
                    </Card.Body>)}
                  </Card>
                </Form>
              </Card.Body>
