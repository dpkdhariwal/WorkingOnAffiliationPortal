...
import LandDocumentsVerification from "@/screens/state/assessor/AssessmentI/LandDocuments/views/LandDocuments";
import LeaseDocumentsVerification from "@/screens/state/assessor/AssessmentI/LandDocuments/views/LeaseDocuments";
import LandAreaVerification from "@/screens/state/assessor/AssessmentI/LandDocuments/views/LandArea";
...
export const LandDocuments = ({ steps, step, view: viewProp = false, isView = false, nav }) => {
    ...
    const onNext = async () => {
        ...
        try {
            ...
            await ap.save_reply_stage_I_verifications(values, step, appId);
            Swal.close();
    ...
    const [initValues, setInitValues] = useState(C.st1Asmt.LandDocuments.intiValues);
    const LoadVerificationStatus = async () => {
        try {
            ...
            Object.entries(resp.data).forEach(([key, value]) => {
                ...
                switch (key) {
                    case C.ASSESSMENT_STAGE_I_KEYS.LEASE_DOCUMENTS:
                        addItem({ row: value, title: "Lease Documents", comp: <LeaseDocumentsVerification title={key} info={value} FormContext={FormContext} /> });
                        break;
                    case C.ASSESSMENT_STAGE_I_KEYS.LAND_DOCUMENTS:
                        addItem({ row: value, title: "Land Documents", comp: <LandDocumentsVerification title={key} info={value} FormContext={FormContext} /> });
                        break;
                    case C.ASSESSMENT_STAGE_I_KEYS.LAND_AREA:
                        addItem({ row: value, title: "Land Area", comp: <LandAreaVerification title={key} info={value} FormContext={FormContext} /> });
                        break;
                }
            });
    ...
    return (
        <Formik innerRef={formRef}
            ...
            validationSchema={C.st1Asmt.LandDocuments.ValSchema}
